...
class CommentResource extends Resource
{
    ...
    {
        return parent::getEloquentQuery()->with('children')->orderBy('id', 'desc')->orderBy('parent_id', 'asc');
    }
    ...
    public static function getNavigationBadgeColor(): ?string
    {
        ...
        $count = static::getModel()::where('status', false)->count();
        $color = 'success';
    ...
    {
        return __('Comments pending moderation');
    }
                        ...
                        Section::make()
                            ...
                            ->schema([
                                ...
                                Placeholder::make('parent.user_id')
                                    ->label('')
                                    ->content(fn(Comment $record): string => ($record->parent_id >= 1 ? __('In response to user') . ' ' . $record->user->name : __('In response to no one')))
                                    ->columnSpanFull(),
                            ]),
                    ]),
            ]);
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('content')
                    ...
                    ->description(fn(Comment $record): string => ($record->parent_id !== null ? __('In response to') . ' ' . $record->parent->user->name : ''), position: 'above')
                    ->limit(50),
