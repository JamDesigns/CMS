<div @class(['section mx-auto section--default' => !$hasHeroImage(), 'relative py-10 bg-light sm:py-20 before:bg-black/25 before:z-10 before:absolute before:inset-0' => $hasHeroImage()])>
    <div>
        <div class="relative z-10 px-4">
            @if($title)
                <h1 @if($hasHeroImage())class="text-white" @else class="text-center" @endif>
                    {{ Statikbe\FilamentFlexibleContentBlocks\FilamentFlexibleContentBlocks::replaceParameters($title) }}
                </h1>
            @endif

            @if($intro)
                <div class="w-full text-lg md:text-xl @if($hasHeroImage()) md:w-2/3 text-white @else container mx-auto @endif [&_a]:underline hover:[&_a]:no-underline">
